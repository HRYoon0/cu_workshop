...
function QuizView({ nickname, quiz, sessionId, session, participantId }: { nickname: string; quiz: any; sessionId: string; session: any; participantId: string }) {
  ...
  return (
    <div className="min-h-screen flex items-start justify-center pt-2 px-3 pb-12">
      <div className="max-w-2xl w-full">
        ...
        {/* 퀴즈 컨텐츠 */}
        <div className="bg-white rounded-b-3xl shadow-2xl p-6 pb-10">
          {!isSubmitted ? (
            <>
              {/* 질문 */}
              <div className="mb-5">
      ...
      <div className="border-t-2 border-gray-200 pt-4">
        ...
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-2xl p-4 mb-3">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-3xl font-bold text-purple-600">{currentScore.toLocaleString()}점</p>
            </div>
            <div>
              <p className="text-gray-600 text-sm mb-1">현재 순위</p>
              <p className="text-3xl font-bold text-blue-600">
                {currentRank > 0 ? `${currentRank}위` : '-'}
              </p>
            </div>
          </div>
          {currentRank > 0 && totalParticipants > 0 && (
            <div className="mt-3 text-sm text-gray-500">
              전체 {totalParticipants}명 중 상위 {Math.round((currentRank / totalParticipants) * 100)}%
            </div>
          )}
        </div>
        ...
        <div className="bg-gray-50 rounded-xl p-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-gray-700">{questionNumber}/{totalQuestions}</span>
          </div>
        </div>
      </div>

      {/* 다음 문제 안내 */}
      <p className="text-gray-600 text-base pt-2 pb-6">
